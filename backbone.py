...
class FPN(nn.Module):
    ...
    def __init__(self, in_channels,fpn_config):
        super().__init__()
        self.fpn_config = fpn_config

        self.lat_layers  = nn.ModuleList(
            [nn.Conv2d(x, self.fpn_config.num_features, kernel_size=1) for x in reversed(in_channels)]
        )

        # This is here for backwards compatability
        padding = 1 if self.fpn_config.pad else 0
        self.pred_layers = nn.ModuleList(
            [nn.Conv2d(self.fpn_config.num_features, self.fpn_config.num_features, kernel_size=3, padding=padding) for _ in in_channels]
        )

        if self.fpn_config.use_conv_downsample:
            self.downsample_layers = nn.ModuleList(
              [nn.Conv2d(self.fpn_config.num_features, self.fpn_config.num_features, kernel_size=3, padding=1, stride=2) for _ in range(self.fpn_config.num_downsample)]
            )
    ...
    def forward(self, convouts):
        ...
        for i in range(len(convouts)):
            ...
            if i > 0:
                ...
                x = F.interpolate(x, size=(h, w), mode=self.fpn_config.interpolation_mode, align_corners=False)
            x = x + self.lat_layers[i](convouts[j])
            out[j] = F.relu(self.pred_layers[i](x))

        # In the original paper, this takes care of P6
        for idx in range(self.fpn_config.num_downsample):
            if self.fpn_config.use_conv_downsample:
...
def get_resize_param(cfg):
    assert cfg.name  == 'ResNet101'
    if cfg.name  == 'ResNet101':
        selected_layers_index = cfg.selected_layers
        return [2**(i+1) for i in selected_layers_index]

class BodyLayer(nn.Module):
    def __init__(self,backbone_config,fpn_config):
        super().__init__()
        self.bkb_config = backbone_config
        self.fpn_config = fpn_config
        self.out_channels = "return it self"
        if self.bkb_config is not None:
            self.backbone        = construct_backbone(backbone_config)
            self.selected_layers = backbone_config.selected_layers
            self.out_channels    = self.backbone.channels
            #self.resize_per_layer= get_resize_param(bkb_config)
            if self.fpn_config is not None:
                # Some hacky rewiring to accomodate the FPN
                self.fpn             = FPN([self.out_channels[i] for i in self.selected_layers],self.fpn_config)
                self.selected_layers = list(range(len(self.selected_layers) + self.fpn_config.num_downsample))
                self.out_channels    = [self.fpn_config.num_features] * len(self.selected_layers)
                # so, if use fpn all the output channels is fpn_config.num_features = 256

    def forward(self, x):
        if self.bkb_config is None:
            return [x]
        ...
        if self.fpn_config is not None:
            ...
            outs = [outs[i] for i in self.bkb_config.selected_layers]
            outs = self.fpn(outs)
